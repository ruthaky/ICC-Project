...
const SuggestedProperty = ({ id }) => {
  ...
  const localStorageData = storedData
    ? storedData.find((property) => property._id === id)
    : null;
  const propertyFromRedux = useSelector((state) => state.propertyDetails.data);
  const PropID = localStorageData
    ? localStorageData.PROP_ID
    : propertyFromRedux
    ? propertyFromRedux.PROP_ID
    : null;
      ...
      if (PropID) {
        try {
          ...
          const similarityResponse = await axios.post(
            `${process.env.REACT_APP_DJANGO_API_URL}recommend/recommendations/${PropID}/`
          );
          ...
          const fetchRecommendation = await axios.post(
            `${process.env.REACT_APP_NODE_API_URL}recommendations/${PropID}/`,
            propertyIDs
          );
          const propertyData = fetchRecommendation.data;

          const combinedData = propertyData.map((property) => {
            const matchedSimilarity = similarityData.find(
              (simProp) => simProp.PropertyID === property.PROP_ID
            );
            return {
              ...property,
          ...
          console.error(
            "Error fetching recommendations or property details:",
            err
          );
              ...
              >
                ...
                <Card.Body>
                  ...
                  <Card.Text>
                    ...
                    {item.PROPERTY_TYPE || "N/A"} <br />
                    <strong>Location:</strong> {item.location || "N/A"} <br />
                    <strong>Bedrooms:</strong> {item.BEDROOM_NUM || "N/A"}{" "}
                    <br />
                    <strong>Area:</strong>{" "}
                    {item.AREA ? `${item.AREA} sq ft` : "N/A"} <br />
                    <strong>Price:</strong> ETB
                    {item.PRICE >= 1
                      ? `${item.PRICE} `
