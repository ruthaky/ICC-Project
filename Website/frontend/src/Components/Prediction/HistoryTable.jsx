      ...
      {history.length > 0 && (
        <Container className="my-5">
          <Row className="justify-content-center">
            <Col lg={12}>
              <Card
                className="shadow-lg border-10"
                style={{ borderColor: "purple", borderWidth: "1px" }}
              >
                <Card.Header
                  style={{
                    background: "#0e1117",
                ...
                <Card.Body className="p-4">
                        ...
                        <tr key={index} className="align-middle">
                          ...
                          <td>
                            ...
                            <p className="text-center mt-4">
                              ...
                              {entry.prediction > 1
                                ? `${(entry.prediction - 0.13).toFixed(
                                    2
                                  )} to ${(entry.prediction + 0.13).toFixed(
                                    2
                                  )} `
                                : // when prediction less than 1 but upper limit > 1

                                entry.prediction + 0.13 > 1
                                ? `${(entry.prediction - 0.13).toFixed(
                                    2
                                  )} to ${(entry.prediction + 0.13).toFixed(
                                    2
                                ...
                                : // in lakh
                                  `${((entry.prediction - 0.13) * 100).toFixed(
                                    2
                                  )} to ${(
                                    (entry.prediction + 0.13) *
                                    100
                                  ).toFixed(2)}  Lakh`}
                            </p>
                          </td>
                  ...
                  <p className="text-center" style={{ color: "red" }}>
                    <strong>
                      Note: Please be aware that predictions may be less
                      accurate when dealing with atypical or unusual inputs.
                    </strong>
                  </p>
                </Card.Body>
              </Card>
